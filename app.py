...
st.title("🍡 청다움 경영 관리 시스템 V8.1")

# --- [3] 데이터 초기화 (앱을 끄기 전까지 유지되는 장부) ---
if 'p_db' not in st.session_state: st.session_state['p_db'] = []
if 'sales' not in st.session_state: st.session_state['sales'] = []

# 구글 시트 데이터 읽기 시도
try:
    df_from_sheet = conn.read(spreadsheet=URL, worksheet="product_db", ttl=0)
    # 시트에 이미 데이터가 있다면 앱의 임시 장부와 합칩니다.
    if not df_from_sheet.empty and not st.session_state['p_db']:
        st.session_state['p_db'] = df_from_sheet.to_dict('records')
    st.success("✅ [청다움_DB] 장부를 읽어왔습니다.")
except:
    st.info("💡 실시간 시트 쓰기는 보안 설정(JSON)이 추가로 필요합니다. 현재는 앱 내부 저장을 사용합니다.")
    ...
    with st.form("p_reg_v81"):
        ...
        if st.form_submit_button("✨ 상품 DB 등록"):
            if p_name:
                ...
                st.success(f"[{p_name}] 등록 성공! (현재 앱 내부 장부에 기록됨)")
                st.rerun()

    if st.session_state['p_db']:
        st.write("### 📋 현재 등록 상품 목록")
        st.table(pd.DataFrame(st.session_state['p_db']).assign(원가=lambda x: x['원가'].map(fmt), 권장가=lambda x: x['권장가'].map(fmt)))

# ==========================================
# 탭 4: 최종 결산 (명칭 고정 및 계산)
# ==========================================
with tabs[3]:
