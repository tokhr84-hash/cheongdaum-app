...
# --- [1] 시스템 설정 ---
st.set_page_config(page_title="청다움 마스터 V12.0", page_icon="🍡", layout="wide")
...
st.title("🍡 청다움 경영 관리 시스템 V12.0")

# 장부 실시간 로드 및 연결 확인
try:
    df_p = conn.read(ttl=0)
    st.success("✅ 구글 시트 금고와 완벽하게 연결되었습니다! (영구 저장 가능)")
except Exception as e:
    st.error(f"⚠️ 연결 확인 중: {e}")
    st.info("💡 Secrets 박스의 설정을 '대청소' 문구와 똑같이 맞췄는지 확인해 주세요.")
    st.stop()
...
# ==========================================
# 탭 1: 상품 정보 등록 (정석 저장 로직)
# ==========================================
with tabs[0]:
    st.subheader("📍 신규 상품 영구 등록")
    with st.form("master_v12_form"):
        c1, c2 = st.columns([2, 1])
        p_name = c1.text_input("📝 상품명", placeholder="예: 앙금플라워 6구")
        target_m = c2.number_input("🎯 목표 마진 (0.4 = 40%)", value=0.4, step=0.1)
        
        st.write("🌿 **[원재료등] 입력**") 
        bom = st.data_editor(pd.DataFrame([{"구분": "원재료", "항목": "", "수량": 0.0, "단가": 0}]), num_rows="dynamic", key="v12_bom")
