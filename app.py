...
# --- [1] 시스템 설정 ---
st.set_page_config(page_title="청다움 마스터 V18.0", page_icon="🍡", layout="wide")

def fmt(val): 
    try: return f"{int(float(str(val).replace(',', ''))):,}"
    except: return str(val)

# --- [2] 정석 연결 (시스템이 Secrets에서 알아서 열쇠를 찾습니다) ---
...
# ==========================================
# 탭 1: 상품 정보 등록 (영구 저장 가동)
# ==========================================
with tabs[0]:
    st.subheader("📍 신규 상품 영구 등록")
    with st.form("v18_master_form"):
        c1, c2 = st.columns([2, 1])
        p_name = c1.text_input("📝 상품명", placeholder="예: 앙금플라워 6구")
        target_m = c2.number_input("🎯 목표 마진 (0.4 = 40%)", value=0.4, step=0.1)
        
        st.write("🌿 **[원재료등] 입력**") 
        bom = st.data_editor(pd.DataFrame([{"구분": "원재료", "항목": "", "수량": 0.0, "단가": 0}]), num_rows="dynamic", key="v18_bom")
...
with tabs[1]:
    ...
    if not df_p.empty and "상품명" in df_p.columns:
        ...
        if cc.button("판매 기록 추가", use_container_width=True):
            rev, net = float(ap)*qty, (float(ap)-float(p_info["원가"]))*qty
